...
public class AnvilCommand extends BrigadierCommand {
    @Override
    public String[] names() {
    ...
    @Override
    public String usage() {
        return "/anvil [<targets>]";
    }

    @Override
    public String description() {
        return "Opens the anvil gui for the targets";
    }

    @Override
    public void literal(LiteralArgumentBuilder<CommandSourceStack> literal) {
        literal.requires(EssentialsUtil.checkPermissions(Permissions.ANVIL_SELF_PERMISSION, Permissions.ANVIL_OTHER_PERMISSION));
        ...
        literal.then(Commands.argument("targets", EntityArgument.players())
                .requires(EssentialsUtil.checkPermissions(Permissions.ANVIL_OTHER_PERMISSION))
    ...
    private static int open(CommandSourceStack source, Collection<ServerPlayer> targetsUnchecked) throws CommandSyntaxException {
        ...
        if (targets.size() == 1) {
            EssentialsUtil.sendSuccess(source, Component.text("Der Amboss wurde für ", Colors.SUCCESS)
