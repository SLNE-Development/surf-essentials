...
public class GrindstoneCommand extends BrigadierCommand {
    @Override
    public String[] names() {
        return new String[]{"grindstone"};
    }

    @Override
    public String usage() {
        return "/grindstone [<targets>]";
    }

    @Override
    public String description() {
        return "Opens the grindstone gui for the targets";
    }

    @Override
    public void literal(LiteralArgumentBuilder<CommandSourceStack> literal) {
        literal.requires(EssentialsUtil.checkPermissions(Permissions.GRINDSTONE_SELF_PERMISSION, Permissions.GRINDSTONE_OTHER_PERMISSION));
        ...
        literal.then(Commands.argument("targets", EntityArgument.players())
                .requires(EssentialsUtil.checkPermissions(Permissions.GRINDSTONE_OTHER_PERMISSION))
    ...
    private static int open(CommandSourceStack source, Collection<ServerPlayer> targetsUnchecked) throws CommandSyntaxException {
        ...
        for (Player target : targets) {
            target.getBukkitEntity().openGrindstone(target.getBukkitEntity().getLocation(), true);
        }

        if (targets.size() == 1) {
            EssentialsUtil.sendSuccess(source, Component.text("Der Schleifstein wurde für ", Colors.SUCCESS)
