...
public class FlySpeedCommand extends BrigadierCommand {
    @Override
    public String[] names() {
        return new String[]{"flyspeed", "speedfly"};
    }

    @Override
    public String usage() {
        return "/flyspeed <speed | default> <players>";
    }

    @Override
    public String description() {
        return "Change the fly speed";
    }

    @Override
    public void literal(LiteralArgumentBuilder<CommandSourceStack> literal) {
        literal.requires(EssentialsUtil.checkPermissions(Permissions.FLY_SPEED_PERMISSION_SELF, Permissions.FLY_SPEED_PERMISSION_OTHER));
    ...
    private int speed(CommandSourceStack source, Collection<ServerPlayer> playersUnchecked, int speed) throws CommandSyntaxException {
        ...
        for (ServerPlayer player : players) {
            ...
            EssentialsUtil.sendSuccess(player, Component.text("Deine Fluggeschwindigkeit wurde auf ", Colors.SUCCESS)
        ...
        if (source.isPlayer()){
            if (successes == 1 && source.getPlayerOrException().getUUID() != players.iterator().next().getUUID()){
                EssentialsUtil.sendSuccess(source, Component.text("Die Fluggeschwindigkeit von ", Colors.SUCCESS)
            ...
            if (successes == 1){
                EssentialsUtil.sendSourceSuccess(source, EssentialsUtil.getDisplayName(players.iterator().next())
                        .append(Component.text("´s fly speed was set to ", Colors.SUCCESS))
                        .append(Component.text(speed, Colors.TERTIARY)));
            }else {
                EssentialsUtil.sendSourceSuccess(source, Component.text("Set the fly speed to ", Colors.SUCCESS)
                        .append(Component.text(speed, Colors.TERTIARY))
