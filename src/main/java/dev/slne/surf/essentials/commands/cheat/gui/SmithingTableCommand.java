...
public class SmithingTableCommand extends BrigadierCommand {
    @Override
    public String[] names() {
        return new String[]{"smithingtable", "smithing"};
    }

    @Override
    public String usage() {
        return "/smithingtable [<targets>]";
    }

    @Override
    public String description() {
        return "Opens the smithingtable gui for the targets";
    }

    @Override
    public void literal(LiteralArgumentBuilder<CommandSourceStack> literal){
        literal.requires(EssentialsUtil.checkRequiredPermissions(Permissions.SMITHING_TABLE_SELF_PERMISSION, Permissions.SMITHING_TABLE_OTHER_PERMISSION));
        ...
        literal.then(Commands.argument("targets", EntityArgument.players())
                .requires(EssentialsUtil.checkPermissions(Permissions.SMITHING_TABLE_OTHER_PERMISSION))
    ...
    private static int open(CommandSourceStack source, Collection<ServerPlayer> targetsUnchecked) throws CommandSyntaxException {
        ...
        for (Player target : targets) {
            target.getBukkitEntity().openSmithingTable(target.getBukkitEntity().getLocation(), true);
        }
        if (source.isPlayer()){
            if (targets.size() == 1){
                EssentialsUtil.sendSuccess(source, Component.text("Der Schmiedetisch wurde für ", Colors.SUCCESS)
